---
title: Private and Protected Attributes
tags:
  - python
  - programming
  - encapsulation
...
class MyClass:
  ...
  def get_protected_attr(self):  # This is not idiomatic in Python, use properties instead
...
- **Use Double Underscore for Name Mangling**: prefix with double underscores
  (e.g., `__private_attr`) only when necessary to avoid name clashes in subclasses. **_But only if necessary_**.
