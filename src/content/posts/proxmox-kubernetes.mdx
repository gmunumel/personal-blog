...
You will see the `EXTERNAL-IP` column populated with an IP address from the pool you configured (e.g., 192.168.2.242).

```bash
NAME                                 TYPE           CLUSTER-IP     EXTERNAL-IP     PORT(S)                      AGE
ingress-nginx-controller             LoadBalancer   10.43.83.206   192.168.2.242   80:31546/TCP,443:30256/TCP   14h
...
This will forward incoming traffic on ports 80 (HTTP) and 443 (HTTPS) to the IP address of your Proxmox server (192.168.2.242).
