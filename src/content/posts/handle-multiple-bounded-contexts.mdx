...
In **_Domain-Driven Design (DDD)_**, a **bounded context** defines the boundaries within which a particular model is defined and applicable.
...
<figure className="flex flex-col items-center w-full">
  <img
    src="/handle-multiple-bounded-contexts/bounded_contexts.png"
    alt="Two Contexts with a common Entity"
    width="600"
  ...
  <figcaption className="text-sm text-gray-500 dark:text-gray-400 mt-1 text-center">
    Two Contexts with a common Entity
  </figcaption>
</figure>

The figure above illustrates two contexts, each with its own model.
However, they share a common entity, which can lead to challenges in maintaining consistency
and communication between the contexts. For _Finance Context_, the `Account` entity might represent a bank account with specific attributes and behaviors,
while for _Human Resources Context_, the same `Account` entity might represent an employee account with different
...
to [Marting Fowler](https://martinfowler.com/bliki/BoundedContext.html): try to do a total unification of these models will not be feasible or cost-effective, which could lead to a monolithic architecture.
...
Usually the issue
arises because both teams use the same _term_ but mean different things in their respective contexts. Refine the **_Ubiquitous Language_**
to avoid this issue.
...
<figure className="flex flex-col items-center w-full">
  <img
    src="/handle-multiple-bounded-contexts/embrace_independence.png"
    alt="Two Contexts with independent Entities"
    width="600"
    className="border bg-white border-gray-200 dark:border-gray-700 shadow-md transition-transform duration-200 hover:scale-[1.02] p-6"
  />
  <figcaption className="text-sm text-gray-500 dark:text-gray-400 mt-1 text-center">
...
In Python the name of the entity can be the same, but they will be defined in different modules or classes.
...
This approach allows you to maintain clear boundaries between the contexts while still being able to use the same name for the entity.
...
It provides a clear separation between different parts of the system, allowing each part to develop individualy while still being able to communicate effectively.
Result would be a coherent dialect of the **_Ubiquitous Language_** across the system.
...
<figure className="flex flex-col items-center w-full">
  <img
    src="/handle-multiple-bounded-contexts/context_map.png"
    alt="Context Map"
    width="600"
    className="border bg-white border-gray-200 dark:border-gray-700 shadow-md transition-transform duration-200 hover:scale-[1.02] p-6"
  />
...
in this case the `Record` entity in _Monitor Context_.

<figure className="flex flex-col items-center w-full">
  <img
    src="/handle-multiple-bounded-contexts/shared_kernel.png"
    alt="Shared Kernel"
    width="600"
    className="border bg-white border-gray-200 dark:border-gray-700 shadow-md transition-transform duration-200 hover:scale-[1.02] p-6"
  />
...
Following the example of the _Finance Context_ and _Human Resources Context_, we can define a `Record` entity in a shared kernel that both contexts can use.
...
This allows both contexts to share a common understanding of the `Record` entity.

A shared kernel is often the **_Core Domain_** of the system, where both teams have to work together to maintain it.
...
entity does not belong to any specific bounded context, but rather exists in its own context. We should promote it to its own bounded
context and treat others as consumers of its API or events.

<figure className="flex flex-col items-center w-full">
  <img
    src="/handle-multiple-bounded-contexts/separate_context.png"
    alt="Separate Context"
    width="600"
    className="border bg-white border-gray-200 dark:border-gray-700 shadow-md transition-transform duration-200 hover:scale-[1.02] p-6"
  />
  <figcaption className="text-sm text-gray-500 dark:text-gray-400 mt-1 text-center">
...
In the example of the _Finance Context_ and _Human Resources Context_, we could have a separate context for the `Account` entity that both contexts can use _Account Management Context_.
They communicate with this context through its API or events, allowing them to remain loosely coupled.

```python
# no-run
# src/domain/services/account_management.py
...
<figure className="flex flex-col items-center w-full">
  <img
    src="/handle-multiple-bounded-contexts/anti_corruption_layer.png"
...
<figure className="flex flex-col items-center w-full">
  <img
    src="/handle-multiple-bounded-contexts/anti_corruption_layer_2.png"
    alt="Zoom in Anti Corruption Layer for Human Resources Context and Account Management Context"
    width="500"
    className="border bg-white border-gray-200 dark:border-gray-700 shadow-md transition-transform duration-200 hover:scale-[1.02] p-6"
  />
  <figcaption className="text-sm text-gray-500 dark:text-gray-400 mt-1 text-center">
...
```python
# no-run
# src/hr/acl/account_adapter.py
from src.hr.acl.account_service import AccountService

class AccountAdapter:
    def __init__(self, account_management_service):
        self.account_management_service = account_management_service

    def create_account(self, hr_account):
        self.account_management_service.create_hr_account(hr_account)

    def remove_account(self, hr_account):
        self.account_management_service.remove_hr_account(hr_account)
```
...
```python
# no-run
# src/hr/acl/account_translator.py
from src.hr.domain.model.account import Account as HrAccount
from src.account_management.domain.model.account import Account as AccountManagementAccount
...
```python
# no-run
# src/hr/domain/service/account_service.py
from src.hr.domain.model.account import Account as HrAccount
from src.account_management.domain.model.account import Account as AccountManagementAccount
from src.hr.acl.account_translator import AccountTranslator
...
class AccountService:
    ...
    def maintain_account(self, hr_account: HrAccount):
        account_management_account = AccountTranslator.hr_to_account_management(hr_account)
        self.account_management_service.create_account(
            account_id=account_management_account.account_id,
            balance=account_management_account.balance,
...
It uses the `AccountTranslator` to translate between the `HrAccount` and `AccountManagementAccount` entities,
ensuring that both contexts can communicate effectively without tight coupling.
...
<figure className="flex flex-col items-center w-full">
  <img
    src="/handle-multiple-bounded-contexts/aggregate.png"
    alt="Aggregate - A cluster of domain objects that can be treated as a single unit."
    width="600"
    className="border bg-white border-gray-200 dark:border-gray-700 shadow-md transition-transform duration-200 hover:scale-[1.02] p-6"
  />
  <figcaption className="text-sm text-gray-500 dark:text-gray-400 mt-1 text-center">
...
<figure className="flex flex-col items-center w-full">
  <img
    src="/handle-multiple-bounded-contexts/domain_events.png"
    alt="Domain Events"
    width="600"
    className="border bg-white border-gray-200 dark:border-gray=-700 shadow-md transition-transform duration-200 hover:scale-[1.02] p-6"
  />
...
The aggregate root will be the `Account` entity in the _Human Resources Context_.
